...
namespace ibis
{
    namespace ui
    {
        ...
        struct OutputNodeWidget::Private
        {
            std::shared_ptr<ftk::Label> label;
            std::shared_ptr<ftk::VerticalLayout> layout;
        };

        void OutputNodeWidget::_init(
            const std::shared_ptr<ftk::Context>& context,
            const std::shared_ptr<ibis::render::Graph>& graph,
            const std::shared_ptr<ibis::render::INode>& node,
            const std::shared_ptr<ftk::IWidget>& parent)
        ...
        std::shared_ptr<OutputNodeWidget> OutputNodeWidget::create(
            const std::shared_ptr<ftk::Context>& context,
            const std::shared_ptr<ibis::render::Graph>& graph,
            const std::shared_ptr<ibis::render::INode>& node,
            const std::shared_ptr<ftk::IWidget>& parent)
        {
            std::shared_ptr<OutputNodeWidget> out(new OutputNodeWidget);
            out->_init(context, graph, node, parent);
            return out;
        }

        std::string OutputNodeWidget::getNodeID()
        {
            return render::OutputNode::getNodeID();
        }

        ftk::Size2I OutputNodeWidget::getSizeHint() const
        {
            return _p->layout->getSizeHint();
        }

        void OutputNodeWidget::setGeometry(const ftk::Box2I& value)
        {
            INodeWidget::setGeometry(value);
            _p->layout->setGeometry(value);
        }

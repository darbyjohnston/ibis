    ...
    namespace ui
    {
        namespace
        {
            ...
            private:
                ...
                std::shared_ptr<ftk::Observer<models::TimeUnits> > _timeUnitsObserver;
            };

            void TimeUnitsPopup::_init(
                const std::shared_ptr<ftk::Context>& context,
                const std::shared_ptr<models::TimeUnitsModel>& model,
                const std::shared_ptr<IWidget>& parent)
            {
                ...
                for (const auto& label : models::getTimeUnitsLabels())
                {
                ...
                _buttonGroup->setCheckedCallback(
                    ...
                    {
                        model->setTimeUnits(static_cast<models::TimeUnits>(index));
                        close();
                    });

                _timeUnitsObserver = ftk::Observer<models::TimeUnits>::create(
                    model->observeTimeUnits(),
                    [this](models::TimeUnits value)
                    {
        ...
        {
            std::shared_ptr<models::TimeUnitsModel> model;
